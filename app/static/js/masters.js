...
{
    const addServiceBtn = document.querySelector('.btn_add_master');
    const delServiceBtn = document.querySelector('.delete-btn');
    const appointmentsTable = document.querySelector('#masters-table');
    ...
    // Проверяем, не добавлено ли уже поле ввода
        if (document.querySelector('.add-master-container')) return;

        // Создаем контейнер для поля ввода и кнопки
        const container = document.createElement('div');
        container.className = 'add-master-container';
        ...
        input.type = 'text';
        input.placeholder = 'Введите мастера';
        ...
        {
            const masterName = input.value.trim();
            if (!masterName)
            {
                showPopup('Пожалуйста, введите мастера', false);
                return;
            }

            // AJAX-запрос к серверу
            fetch(`/add/master?master_name=${masterName}`,
            {
                method: 'POST',
                headers: {
            ...
            .then(data =>
            {
                if (data.success)
                {
                    ...
                    `;
                    document.querySelector('#masters-table tbody').appendChild(newRow);
                    // Очищаем поле ввода
                    input.value = '';
...
function deleteMaster(id) {
    fetch(`/delete/master/?master_id=${id}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        }
    ...
    .then(data => {
        if (!data.success) {
            showPopup('Не удалось удалить мастера', false);
        }
    });
}
    ...
    buttons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (row) {
                ...
                showPopup('Мастер успешно удален', true);
            }
        });
    });
});